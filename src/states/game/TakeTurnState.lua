    ...
    Timer.after(0.5, function()
        ...
        :finish(function()
            ...
            :finish(function()
                ...
                local dmg = math.max(1, attacker.Attack - defender.Def)
...
function TakeTurnState:victory()
    ...
    :finish(function()
        ...
        function()
            ...
            local exp = (self.opponentPokemon.IVHP + self.opponentPokemon.IVAttack +
                self.opponentPokemon.IVDef + self.opponentPokemon.IVSpeed) * self.opponentPokemon.level
            ...
            Timer.after(1.5, function()
                ...
                Timer.tween(0.5, {
                    [self.battleState.playerExpBar] = {value = math.min(self.playerPokemon.currentXP + exp, self.playerPokemon.expToLevel)}
                })
                :finish(function()
                    ...
                    self.playerPokemon.currentXP = self.playerPokemon.currentXP + exp

                    -- level up if we've gone over the needed amount
                    if self.playerPokemon.currentXP > self.playerPokemon.expToLevel then
                        
                        gSounds['levelup']:play()

                        -- set our exp to whatever the overlap is
                        self.playerPokemon.currentXP = self.playerPokemon.currentXP - self.playerPokemon.expToLevel
                        levels = self.playerPokemon:levelUp()
                        gStateStack:push(BattleMenuState(VIRTUAL_WIDTH - 100, 0, 100, VIRTUAL_HEIGHT - 48, 
                                                        "HP + " + levels[0] + 
                                                        "ATT + " + levels[1] + "\n"+
                                                        "DEF + " + levels[2] + "\n"+ 
                                                        "SAT + " + levels[3] + "\n"+ 
